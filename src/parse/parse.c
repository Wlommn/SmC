...
char *infix_to_postfix(char *infix) {
  ...
  for (int i = 0; i < len; i++) {
    ...
    if (ch == ' ') {
      continue;
    } else if (isdigit(ch) || ch == '.' || ch == ',' || ch == 'x' ||
               ch == 'X' || ch == 'P') {
      if (ch == '.' || ch == ',') {
        postfix[j++] = dbl_sep;
      } else {
        if (j > 0 && !isdigit(postfix[j - 1]) && postfix[j - 1] != dbl_sep &&
            !isspace(postfix[j - 1])) {
          postfix[j++] = ' ';
        }
        postfix[j++] = ch;
        if ((i + 1) < len && !isdigit(infix[i + 1]) && infix[i + 1] != '.' &&
            infix[i + 1] != ',' && !isspace(postfix[j - 1])) {
          postfix[j++] = ' ';
        }
        if (ch == 'x' || ch == 'X') {
          is_function = true;
      ...
      i += 2;
    } else if (ch == 'a' && infix[i + 1] == 's' && infix[i + 2] == 'i' &&
               infix[i + 3] == 'n') {
      Push_Stack(operator_stack, asin_op);
      i += 3;
    } else if (ch == 'a' && infix[i + 1] == 'c' && infix[i + 2] == 'o' &&
               infix[i + 3] == 's') {
      Push_Stack(operator_stack, acos_op);
      i += 3;
    } else if (ch == 'a' && infix[i + 1] == 't' && infix[i + 2] == 'a' &&
               infix[i + 3] == 'n') {
      Push_Stack(operator_stack, atan_op);
      i += 3;
    } else if (ch == 's' && infix[i + 1] == 'q' && infix[i + 2] == 'r' &&
               infix[i + 3] == 't') {
